...
2. `<h3>📏 가장 긴 프리픽스가 이긴다</h3>` — 라우팅 테이블은 목적지 대역별 표이고, 여러 줄이 매치하면 **가장 구체적인(프리픽스가 긴) 줄**이 이긴다. `local` 항목(VPC CIDR)은 VPC를 만들 때 자동으로 들어가고 **지울 수 없다.**
   다만 **타깃은 바꿀 수 있다** — AWS 문서가 "You can change the target of the default local route"라고 명시한다. `local`을 ENI나 Gateway Load Balancer 엔드포인트로 바꾸면 **VPC 내부 통신까지 검사 장비로 우회**시킬 수 있고, 이게 동서(east-west) 트래픽 검사의 표준 방식이다. 9장이 이 문을 쓴다.
3. 데모 (Step 2)
...
(function(){
  ...
  const TABLE = [
    { dest:"10.0.0.0/16",  target:"local",           len:16, fixed:true,
      note:"VPC를 만들 때 자동으로 들어가고 <b>지울 수 없습니다</b>. 기본값으로는 VPC 안 통신이 전부 여기로 갑니다 — 다만 <b>타깃은 바꿀 수 있어요</b>(<a class='link' href='#nfw'>9장</a>)." },
  ...
  const CASE = {
    same: { ip:"10.0.1.42", hit:0,
      ...
      x:"이 경로는 <b>지울 수 없습니다.</b> 그래서 VPC 내부 격리의 일상적인 도구는 <b>SG와 NACL</b>이에요 — 라우팅 테이블에서 이 줄을 빼는 방식으로는 못 막습니다. <br><br>단, <b>타깃을 바꾸는 건 가능합니다.</b> <code>local</code>의 타깃을 검사 장비의 ENI로 교체하면 VPC 안 통신까지 그 장비를 거치게 만들 수 있어요. 규모가 커지면 쓰는 수법이고, <a class='link' href='#nfw'>9장</a>에서 다시 만납니다." },
