...
- **원인은 아직 규명되지 않았다.** 그 값이 **왜** 문자열이 되었는지도, 그 문자열이 **무엇이었는지도** 단정해서 쓰지 않는다. 스택트레이스가 보증하는 범위는 타입까지다. 2장 전체가 "우리는 아직 모른다"는 사실 위에 서 있으므로, 앞 장에서 내용을 아는 것처럼 쓰면 다음 장이 자기 앞 장과 모순된다. 4장은 전형적인 세 경로를 **가설로** 나란히 놓을 뿐 하나를 고르지 않는다.
  **3부(11~14장)에는 이 제약이 특히 위험한 형태로 나타난다.** 우리는 그 기획서에 무엇이 적혀 있었는지도, 어떤 대화가 오갔는지도, 누가 무엇을 물었는지도 모른다. 3부는 **일반적인 구조**(요건 문서가 정상 경로로 채워지는 이유, 물어야 할 질문, 문장의 형식)를 다루는 곳이지 오늘의 기획 과정을 재구성하는 곳이 아니다. 오늘 사건에 대해 3부가 쓸 수 있는 것은 **코드에 그 결정이 없었다**는 것뿐이다.
...
- **`.stack` 규칙.** 스프링과 라이브러리 이름은 `<details class="stack">` 안에만 둔다. **표제는 "코드에서는 이 이름입니다" 로 통일한다** — 블록에 담기는 것이 스프링만이 아니기 때문이다(`record` 와 정적 팩터리는 순수 자바이고, Resilience4j 는 라이브러리다). 클래스 이름이 `.spring` 이 아니라 `.stack` 인 이유도 같다. 그 블록 안에 `<h2>` · `<h3>` · `.demo` · `.quiz` · `.nextq` 를 넣지 않는다. 블록을 전부 접어도 논지가 끊기지 않아야 한다.
  **블록 안의 모든 문장은 본문이 이미 세운 개념에 이름과 자리를 붙이는 문장이어야 한다.** 판별법은 "그 문장을 지웠을 때 독자가 무엇을 잃는가"이고, 잃는 것을 **셋으로 가른다.**
  - **이름과 자리** — 그 개념이 이 스택에서 무엇으로 불리고 어느 자리에 사는가. **허용한다.** 그것이 이 블록의 목적이다. 자리를 말하지 않고 이름만 나열하면 문장이 아니라 표가 된다.
  - **존재 이유** — 왜 그렇게 해야 하는가. **위반이다.** 2장 초안의 "로그를 남기는 코드가 특정 로깅 구현에 매이지 않게 해 주는 계층이…" 가 이 경우였다.
  - **새 결정 축** — 본문에 없던 판단거리를 하나 더 여는 것. **위반이다.** 4장 초안의 "모르는 필드를 만났을 때 조용히 무시할지 즉시 실패할지…" 가 이 경우였다.

  통과한 본보기는 2장의 블록이다. "**이 세 조각을** 실제로 로그로 남기는 코드는 보통 SLF4J 같은 로깅 파사드를 거칩니다. **방금 말한 '무엇에 대해'의 식별자를** 요청 하나에 속한 모든 로그 줄에 자동으로 붙여 주는 자리가 MDC입니다." 두 문장 다 본문의 표현을 그대로 집어 든 뒤 이름과 자리를 붙인다.

  아래 각 장의 `.stack` 항목에는 **그 항목이 매달릴 본문 개념**을 함께 적어 두었다. 구현할 때 그 개념이 본문에 실제로 세워졌는지 확인하고, 세워지지 않았다면 항목을 빼고 **부록 A** 로 보낸다. 부록은 사슬 밖의 참조물이라 개념 축이 하나 더 늘어도 논지가 끊기지 않는다. 기계 검사는 이 규칙을 잡지 못하므로 사람이 본다.
- **장당 `.oneline` 은 하나다.** 이 문서는 장마다 강조 상자(`.oneline`)를 정확히 하나씩 두는 관례로 자랐다. 그 장이 남길 문장 하나를 고르라는 뜻이다. 둘을 두면 무엇이 그 장의 문장인지 흐려진다. 더 강조하고 싶은 것이 있으면 본문의 `<b>` 로 내린다.
...
6. `<details class="stack">`: Bean Validation 과 `@Valid` 가 서는 자리 ← 본문의 "경계 검증의 최소 형태(필수 항목이 비어 있는지, 기대한 타입이 맞는지)". `Objects.requireNonNull` ← 같은 개념.
   **역직렬화의 알 수 없는 필드 처리는 넣지 않는다.** 그것은 "누락"·"타입 불일치"와 다른 세 번째 축이고 이 장의 본문이 세우지 않는다. 부록 A 로 보낸다.
...
8. `<details class="stack">`: 레코드 ← 본문의 "불변 객체와 값 타입". 정적 팩터리 메서드 ← 본문의 "생성자에서의 강제"(생성자를 대신하는 자리이므로 같은 개념에 매달린다).
   **설정 바인딩은 넣지 않는다.** 이 장의 본문에 설정 바인딩 이야기가 없다. 부록 A 로 보낸다.
...
6. `<details class="stack">`: JSP 에서 include 를 나누는 방법 ← 본문의 "격리 단위와 조립 단위가 어긋나 있다". `<c:catch>` ← 본문의 "응답 버퍼가 이미 커밋되었다면 오류 페이지로 넘기는 것조차 불가능하다".
   **`<c:catch>` 의 한계를 이 블록에서 새로 설명하지 않는다.** 본문이 세운 그 사실에 이름만 붙이고, 자세한 것은 부록 B 로 넘긴다.
...
7. `<details class="stack">`: `@ControllerAdvice` · `@ExceptionHandler` ← 본문의 "전역 예외 처리기는 마지막 그물이지 방어가 아니다".
   **트랜잭션 경계와 예외의 관계는 넣지 않는다.** 이 장의 본문이 트랜잭션을 세우지 않는다. 부록 A 로 보낸다.
...
7. `<details class="stack">`: Resilience4j 의 `TimeLimiter` · `Bulkhead` · `CircuitBreaker` · `Retry` ← 본문의 타임아웃·격벽·회로 차단·재시도 네 개념에 하나씩 매달린다.
   **"타임아웃이 연결과 읽기 둘"이라는 사실은 `.stack` 이 아니라 본문(위 4번)에 넣는다.** 타임아웃 값을 정하는 법을 말하면서 어느 타임아웃인지 말하지 않으면 그 설명이 불완전하다. 이름이 아니라 이해에 속하는 내용이므로 본문의 것이다.
...
3. **이번 사건을 여기서 다루되 단정하지 않는다.** 우리는 그 기획서에 무엇이 적혀 있었는지 모른다. 요건이 있었는데 구현되지 않았을 수도 있다. **아는 것은 하나다. 코드에 그 결정이 없었다.** 그래서 이렇게 쓴다: 코드에 없었던 이유는 둘 중 하나다. 요건이 없었거나, 있었는데 옮겨지지 않았거나. **둘 중 무엇이든 이 장의 논지는 성립한다** — 요건이 없었다면 아무도 그것을 물어야 한다고 생각하지 않은 것이고, 있었는데 옮겨지지 않았다면 그 요건이 검수 대상에 오르지 않은 것이다. 실패 요건은 화면이 없어서 두 경우 다 조용히 지나간다.
