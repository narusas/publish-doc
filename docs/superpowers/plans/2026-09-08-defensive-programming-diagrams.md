...
```css
/* 흐리게 하는 데 opacity 를 쓰면 안 된다. 구동기의 .dia [data-at]{opacity:0} 과
   .dia [data-at].vis{opacity:1} 이 같은 속성을 쥐고 있어서, 보일 때나 숨을 때나
   이 선언은 한 번도 이기지 못한다. 아무도 안 쥔 fill-opacity 로 같은 효과를 낸다. */
.dia .gone{fill:var(--text-mut); fill-opacity:.5}
...
wireDia('diaCommit', [
  ...
  {c:'앞쪽 태그들이 흘려보낸 뒤 141번째에서 예외가 났다면, 그 예외는 커밋보다 늦은 자리에서 난 것입니다.', h:3400},
...
wireDia('diaLayers', [
  ...
  {c:'네 층을 나란히 놓으면 자격이 높이를 따라가지 않습니다. 잡는 동작이 같아 보여도, <b>대안이 없으면 그것은 방어가 아니라 삼킴입니다.</b>', h:4400},
]);
```
  ...
  <div class="dia-scroll">
    <svg viewBox="0 0 720 270" role="img" aria-label="타임아웃 값을 상위 예산에서 거꾸로 나누는 방향을 막대로 보여 주는 그림. 먼저 메인페이지가 지켜야 하는 1000밀리초를 세우고, 그 안에서 섹션 하나가 쓸 몫을 300밀리초로 정한 다음, 그 300밀리초를 다시 연결에 100밀리초와 읽기에 200밀리초로 나눈다. 어디서 본 3000밀리초를 그대로 옮겨 쓰면 섹션 하나가 전체 예산의 세 배를 쓰는 셈이 된다. 반대로 너무 빠듯하게 잡으면 평소 100밀리초짜리 호출이 150밀리초로 흔들리는 정상 범위까지 실패로 집계된다.">
      ...
      <g data-at="2+">
        ...
        <rect fill="var(--ok)" opacity=".11" stroke="var(--ok)" x="105" y="168" width="130" height="28" rx="6"/>
      ...
      <g data-at="3">
        ...
        <!-- 이 막대만 축척을 벗어나면 그림이 재는 것과 말하는 것이 달라진다. 3000ms 를 같은
             축척(1ms=0.65px)으로 그리면 1950px 이고, viewBox 밖은 svg 가 잘라 낸다.
             잘린 끝이 "예산 밖으로 나간다"를 그대로 보여 준다. 폭이 viewBox 보다 큰 것은
             의도이므로, 넘침을 재는 검사는 이 사각형을 예외로 두어야 한다. -->
        <rect fill="var(--bad)" opacity=".18" stroke="var(--bad)" x="40" y="230" width="1950" height="28" rx="6"/>
        <text class="m no" x="352" y="249" text-anchor="middle">3000ms · 이 그림의 오른쪽 끝을 지나 예산의 세 배까지 갑니다</text>
      </g>
...
function paintRadiusDia(root){
  ...
  for(let k = RADIUS.length - 1; k >= 0; k--){
    ...
    if(k > 0){
      base.push(`<text class="s" x="${x + 10}" y="${y + 16}">반경 ${k + 1} · ${RADIUS[k].n}</text>`);
      base.push(`<text class="cap-l" x="${x + 10}" y="${y + 32}">${RADIUS[k].ch}장</text>`);
    } else {
...
wireDia('diaRadius', RADIUS.map(r => ({
  ...
  // 문구는 RADIUS 에서 그대로 가져온다. 그 배열은 지도 카드용이라 해라체이므로 따옴표로
  // 묶어 인용임을 드러내고, 문장의 뼈대는 합쇼체로 세운다. 배열을 고치면 개요의 카드와
  // 어긋나고, 그 단일 출처가 이 문서가 지키는 불변식이다.
  c: '이 반경이 하는 일은 <b>"' + r.can + '"</b>입니다.'
     + (r.hole === '여기가 종착지다'
          ? ' <span class="ye">여기가 종착지입니다. 이 반경 밖에는 코드가 없습니다.</span>'
          : ' <span class="no">그럼에도 "' + r.hole + '"는 것이 남습니다.</span>'),
  h: 4000,
})));
```
